		...
		else
		{
			...
			else
			{
				...
				for (;;)
				{
					...
					if (Fileinfo.fattrib & AM_ARC) // is a file?
					{
						if ((strstr(Fileinfo.lfname, "wav")) || (strstr(Fileinfo.lfname, "WAV")) || (strstr(Fileinfo.lfname, "MP3")) || (strstr(Fileinfo.lfname, "mp3"))) // is a music file?
						{
							strcpy((char *)FileList.file[FileList.ptr].name, (char *)Fileinfo.lfname);
							FileList.ptr++;
						}
...
void show_filename()
{
	...
	for (uint8_t i = 0; i < num; i++)
	{
		...
		printf("music unicode first:0x%04x\n", FileList.file[i].unicode[0]);
	}
}
...
uint8_t GBK2Unicode(uint8_t ptr)
{
	...
	uint8_t namesize;
	namesize = strlen((char *)FileList.file[ptr].name);
