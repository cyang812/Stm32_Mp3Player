...
uint8_t Mp3_GetID3V2(void) // to get mp3 audio Frame start address store in labelSize
{
	...
	labelSize = (((pLabel->Size)[0] & 0x7F) * 0x200000UL) // ��ȡ��ǩ��С��
				+ (((pLabel->Size)[1] & 0x7F) * 0x4000UL) // һ���ĸ��ֽڣ�����ÿ���ֽ����λΪ0
				+ (((pLabel->Size)[2] & 0x7F) * 0x80UL)	  // ��˹�28λ����ǩ��С�ļ��㹫ʽ�������ʾ
				+ ((pLabel->Size)[3] & 0x7F) + 10;		  // ���ڱ�ǩͷ�е���ʾ�ı�ǩ��С��������ǩͷ�����Ĵ�С������Ҫ���ϱ�ǩͷ��10���ֽ�
	printf("labelSize: %d\n", labelSize);
	f_lseek(&MusicFile, labelSize); // �ƶ��ļ�ָ�뵽��Ƶ���ݴ�
	return true;
}
...
void Mp3Play_init(void)
{
	...
	// if(init==0)//����MP3֡��Ϣ��ʼ����Ƶ�ӿ�
	{
		...
		duration = MusicFile.fsize * 8 / mp3FrameInfo.bitrate; // ���������ʱ�����ļ���С(Byte) * 8 / �����ʣ�bps��
...
mp3_play_status Mp3_SubDecode(void)
{
	...
	{
		memmove(readBuf, readPtr, bytesLeft); // ��readPtr �е����ݰ��Ƶ� readBuf �У�����ͬ��ͷ֮������ݰ��Ƶ�buffͷ��
