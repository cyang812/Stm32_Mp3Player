...
static AUDIO_PLAYBACK_State  audio_state;
__IO uint32_t Volume = 20;
__IO uint32_t PauseEnabledStatus = 0;
...
void AudioPlay_demo (void)
{
  ...
  PauseEnabledStatus = 1; /* 0 when audio is running, 1 when Pause is on */
...
/*  if(BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_SPEAKER, uwVolume, *AudioFreq_ptr) == 0)
  ...
  if(BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_BOTH, Volume, *AudioFreq_ptr) == 0)
  {
  ...
  */
  ...
  sprintf((char*)FreqStr, "       VOL:    %3lu     ", Volume);
...
{
  uint32_t bytesread;
  AUDIO_ErrorTypeDef error_state = AUD_ERROR_NONE;
  ...
  default:
    error_state = AUD_ERROR_NOTREADY;
    break;
  }
...
{
	printf("play buffer 1\n");
  if(audio_state == AUD_STATE_PLAYING)
  {
