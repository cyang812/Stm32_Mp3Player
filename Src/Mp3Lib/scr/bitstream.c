...
int UnpackFrameHeader(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
	...
	#ifdef MP3_SECURITY
	if(bPlaySecurityFile)
	{
		if (!mp3DecInfo || !mp3DecInfo->FrameHeaderPS || (buf[0] ) != 0xfa || (buf[1] & SYNCWORDL) != SYNCWORDL)
			return -1;
	}
	else
	{
	...
	fh = ((FrameHeader *)(mp3DecInfo->FrameHeaderPS));
	//d_printf("fh->buff:");d_printfhex(buf[0]);d_printf(" ");d_printfhex(buf[1]);d_printf(" ");d_printfhex(buf[2]);d_printf(" ");d_printfhex(buf[3]);d_printf(" ");d_printf("\n");
	...
	//d_printf("fh->ver=");d_printfhex(fh->ver);d_printf("\n");
	//d_printf("fh->layer=");d_printfhex(fh->layer);d_printf("\n");
	//d_printf("fh->paddingBit=");d_printfhex(fh->paddingBit);d_printf("\n");
	//d_printf("fh->srIdx=");d_printfhex(fh->srIdx);d_printf("\n");
	//d_printf("fh->brIdx=");d_printfhex(fh->brIdx);d_printf("\n");
	//d_printf("fh->sMode=");d_printfhex(fh->sMode);d_printf("\n");
	#ifdef MP3_SECURITY
	if(bPlaySecurityFile)
	{
		//fh->srIdx=SrIdx_Table[fh->ver][fh->srIdx];
		if(fh->layer==3)
		{
			fh->layer=1;
		}
		...
		//fh->brIdx=10;
		//fh->brIdx=BrIdx_Table[fh->ver][fh->layer - 1][fh->brIdx];
		//d_printf("fh->srIdx=");d_printfhex(fh->srIdx);d_printf("\n");
		//d_printf("fh->brIdx=");d_printfhex(fh->brIdx);d_printf("\n");
		//d_printf("sample=");d_printfhex32(samplerateTab[fh->ver][fh->srIdx]);d_printf("\n");
		//d_printf("bitrate=");d_printfhex32(((int)bitrateTab[fh->ver][fh->layer - 1][fh->brIdx]) * 1000);d_printf("\n");
